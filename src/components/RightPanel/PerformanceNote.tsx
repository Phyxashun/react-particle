...
export interface PerformanceNoteProps {
  className?: string;
}

const PerformanceNote: React.FC<PerformanceNoteProps> = ({ className = '' }: PerformanceNoteProps) => {
  const { mode } = useMode();
  const note = MODES[mode!]?.perfNote;
  if (!note) return null;

  return (
    <div className={`${className} border-success/15 mb-16 rounded border bg-(--color-bg) px-2.5 py-2`}>
